...
impl DependencyAnalyzer {
    ...
    /// Set libraries to be excluded
    ///
    /// Excludes are glob patterns are are treated as absolute paths,
    /// not relative to `root`
    pub fn exclude_libraries(mut self, excludes: Vec<String>) -> Result<Self, glob::PatternError> {
    ...
    fn is_excluded(&self, lib: &Library) -> bool {
        for exclude in self.excludes.iter() {
            if exclude.matches_path(&lib.path) {
                return true;
            }
        }
        false
